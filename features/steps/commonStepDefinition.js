var commonStepDefinition = function() {
    ...
    Given(/I visit "([^"]*)"/, function(type, next) {
        ...
        switch(true) {
            case (/^auction/i.test(type)):
                stand = require(__base + constants.PO.auction);
                // console.log(__base + constants.PO.auction)
                if (/auction-(.*)/.test(type)) {
                    param = type.replace(/auction-(.*)/, '$1');
                    // console.log(param);
                }//end if
                break;
        }//end switch
